...
/// <summary>
/// EtsyAdapter unit testleri — HTTP çağrısı yapmadan temel davranış kontrolü.
/// G015: Etsy adapter 0 test → temel TestConnection/Ping/Properties.
/// </summary>
[Trait("Category", "Unit")]
[Trait("Layer", "Adapter")]
[Trait("Platform", "Etsy")]
public class EtsyAdapterUnitTests
{
    private readonly Mock<ILogger<EtsyAdapter>> _logger = new();

    private EtsyAdapter CreateAdapter(EtsyOptions? options = null)
    {
        var httpClient = new HttpClient(new FakeHandler());
        if (options != null)
            return new EtsyAdapter(httpClient, _logger.Object, Options.Create(options));
        return new EtsyAdapter(httpClient, _logger.Object);
    }

    private static EtsyOptions ValidOptions() => new()
    {
        ShopId = "12345",
    ...
    {
        // Adapter oluşturulmuş ama credential verilmemiş
        var adapter = CreateAdapter();
    ...
    {
        // FakeHandler her zaman exception fırlatır — ping false dönmeli
        var adapter = CreateAdapter(ValidOptions());
        var result = await adapter.PingAsync();
        result.Should().BeFalse();
    }

    /// <summary>Her zaman exception fırlatan fake HTTP handler — network failure simülasyonu.</summary>
