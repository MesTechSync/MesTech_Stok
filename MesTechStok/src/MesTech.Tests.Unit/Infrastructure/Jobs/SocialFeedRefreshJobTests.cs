...
/// Job depends on AppDbContext (concrete) so full integration requires InMemory DB.
...
public class SocialFeedRefreshJobTests
{
    ...
    public async Task ExecuteAsync_NoActiveConfigs_SkipsGracefully()
    {
        ...
        var sut = new SocialFeedRefreshJob(db, new[] { adapter.Object }, Mock.Of<ILogger<SocialFeedRefreshJob>>());
    ...
    public async Task ExecuteAsync_ActiveConfig_CallsMatchingAdapter()
    {
        ...
        using var db = CreateInMemoryDb(tenantId);
