...
public class ReturnCreatedNotificationHandlerTests
{
    private readonly Mock<INotificationLogRepository> _notifRepoMock = new();
    private readonly Mock<IUnitOfWork> _uowMock = new();
    private readonly Mock<ILogger<ReturnCreatedNotificationHandler>> _loggerMock = new();

    private ReturnCreatedNotificationHandler CreateHandler() =>
    ...
    public async Task HandleAsync_HappyPath_ShouldCreateNotificationAndSave()
    {
        ...
        await handler.HandleAsync(
            ...
            PlatformType.Trendyol, ReturnReason.DefectiveProduct,
    ...
    public async Task HandleAsync_DifferentPlatforms_ShouldCreateNotification(PlatformType platform)
    {
        var handler = CreateHandler();

        await handler.HandleAsync(
            Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(),
            platform, ReturnReason.WrongProduct,
            CancellationToken.None);
