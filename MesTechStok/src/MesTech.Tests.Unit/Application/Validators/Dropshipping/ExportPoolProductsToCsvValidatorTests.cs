...
{
    private readonly ExportPoolProductsToCsvValidator _validator = new();

    private static ExportPoolProductsToCsvCommand ValidCommand() => new(
        PoolId: Guid.NewGuid(),
    ...
    [Fact]
    public async Task ZeroMarkup_Passes()
    {
        var cmd = ValidCommand() with { PriceMarkupPercent = 0m };
        var result = await _validator.ValidateAsync(cmd);
        result.IsValid.Should().BeTrue();
    }
}
