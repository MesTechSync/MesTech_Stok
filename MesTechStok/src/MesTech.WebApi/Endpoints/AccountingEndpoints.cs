...
public static class AccountingEndpoints
{
    public static void Map(WebApplication app)
    {
        ...
        // ═══ G207-DEV6 TUR4: 15 eksik Accounting query endpoint ═══

        // GET /api/v1/accounting/fixed-expenses — sabit gider listesi
        group.MapGet("/fixed-expenses", async (
            Guid tenantId, bool? isActive,
            ISender mediator, CancellationToken ct) =>
        {
            var result = await mediator.Send(new GetFixedExpensesQuery(tenantId, isActive), ct);
            return Results.Ok(result);
        })
        .WithName("GetFixedExpenses")
        .WithSummary("Sabit gider listesi (kira, abonelik, vb.)").Produces(200)
        .CacheOutput("Lookup60s");

        // GET /api/v1/accounting/fixed-expenses/{id}
        group.MapGet("/fixed-expenses/{id:guid}", async (
            Guid id, ISender mediator, CancellationToken ct) =>
        {
        ...
        .WithSummary("Tekil sabit gider detayı").Produces(200).Produces(404);

        // GET /api/v1/accounting/salary-records — maaş kayıtları
        group.MapGet("/salary-records", async (
            Guid tenantId, int? year, int? month,
            ISender mediator, CancellationToken ct) =>
        {
            var result = await mediator.Send(new GetSalaryRecordsQuery(tenantId, year, month), ct);
            return Results.Ok(result);
        })
        .WithName("GetSalaryRecords")
        .WithSummary("Maaş kayıtları listesi (yıl/ay filtreli)").Produces(200)
        .CacheOutput("Lookup60s");

        // GET /api/v1/accounting/salary-records/{id}
        group.MapGet("/salary-records/{id:guid}", async (
            Guid id, ISender mediator, CancellationToken ct) =>
        {
        ...
        .WithSummary("Tekil maaş kaydı detayı").Produces(200).Produces(404);

        // GET /api/v1/accounting/penalty-records — ceza kayıtları
        group.MapGet("/penalty-records", async (
            Guid tenantId, PenaltySource? source,
            ISender mediator, CancellationToken ct) =>
        {
            var result = await mediator.Send(new GetPenaltyRecordsQuery(tenantId, source), ct);
            return Results.Ok(result);
        })
        .WithName("GetPenaltyRecords")
        .WithSummary("Ceza kayıtları listesi (kaynak filtreli)").Produces(200)
        .CacheOutput("Lookup60s");

        // GET /api/v1/accounting/penalty-records/{id}
        group.MapGet("/penalty-records/{id:guid}", async (
            Guid id, ISender mediator, CancellationToken ct) =>
        {
        ...
        .WithSummary("Tekil ceza kaydı detayı").Produces(200).Produces(404);

        // GET /api/v1/accounting/tax-records — vergi kayıtları
        group.MapGet("/tax-records", async (
            Guid tenantId, string? taxType, int? year,
            ISender mediator, CancellationToken ct) =>
        {
            var result = await mediator.Send(new GetTaxRecordsQuery(tenantId, taxType, year), ct);
            return Results.Ok(result);
        })
        .WithName("GetTaxRecords")
        .WithSummary("Vergi kayıtları listesi (tür/yıl filtreli)").Produces(200)
        .CacheOutput("Lookup60s");

        // GET /api/v1/accounting/tax-records/{id}
        group.MapGet("/tax-records/{id:guid}", async (
            Guid id, ISender mediator, CancellationToken ct) =>
        {
        ...
        .WithSummary("Tekil vergi kaydı detayı").Produces(200).Produces(404);

        // GET /api/v1/accounting/income-expense-list — gelir/gider listesi
        group.MapGet("/income-expense-list", async (
            Guid tenantId, string? type, DateTime? from, DateTime? to,
            int page, int pageSize,
            ISender mediator, CancellationToken ct) =>
        {
            var result = await mediator.Send(
                new GetIncomeExpenseListQuery(tenantId, type, from, to,
                    Math.Max(1, page), Math.Clamp(pageSize <= 0 ? 20 : pageSize, 1, 100)), ct);
            return Results.Ok(result);
        })
        .WithName("GetIncomeExpenseList")
        .WithSummary("Gelir/gider listesi (sayfalanmış, filtreli)").Produces(200);

        // GET /api/v1/accounting/income-expense-summary — gelir/gider özeti
        group.MapGet("/income-expense-summary", async (
            Guid tenantId, DateTime? from, DateTime? to,
            ISender mediator, CancellationToken ct) =>
        {
            var result = await mediator.Send(new GetIncomeExpenseSummaryQuery(tenantId, from, to), ct);
            return Results.Ok(result);
        })
        .WithName("GetIncomeExpenseSummary")
        .WithSummary("Gelir/gider toplam özeti").Produces(200)
        .CacheOutput("Dashboard30s");

        // GET /api/v1/accounting/fixed-assets — duran varlık listesi
        group.MapGet("/fixed-assets", async (
            Guid tenantId, bool? isActive,
            ISender mediator, CancellationToken ct) =>
        {
            var result = await mediator.Send(new ListFixedAssetsQuery(tenantId, isActive), ct);
            return Results.Ok(result);
        })
        .WithName("ListFixedAssets")
        .WithSummary("Duran varlık listesi (aktif/pasif filtreli)").Produces(200)
        .CacheOutput("Lookup60s");

        // GET /api/v1/accounting/fixed-assets/{id}/depreciation — amortisman hesabı
        group.MapGet("/fixed-assets/{id:guid}/depreciation", async (
            Guid id, ISender mediator, CancellationToken ct) =>
        {
            var result = await mediator.Send(new CalculateDepreciationQuery(id), ct);
            return Results.Ok(result);
        })
        .WithName("CalculateDepreciation")
        .WithSummary("Duran varlık amortisman hesabı").Produces(200);

        // GET /api/v1/accounting/babs-report — Ba/Bs raporu
        group.MapGet("/babs-report", async (
            Guid tenantId, int year, int month,
            ISender mediator, CancellationToken ct) =>
        {
            var result = await mediator.Send(new GenerateBaBsReportQuery(tenantId, year, month), ct);
            return Results.Ok(result);
        })
        .WithName("GenerateBaBsReport")
        .WithSummary("Ba/Bs formu raporu (aylık)").Produces(200)
        .CacheOutput("Lookup60s");
        ...
        group.MapGet("/kdv-report", async (
            ...
            ISender mediator, CancellationToken ct) =>
        {
            var result = await mediator.Send(new GetKdvReportQuery(tenantId, year, month), ct);
            return Results.Ok(result);
        })
        .WithName("GetKdvReport")
        .WithSummary("KDV raporu (aylık beyanname detayı)").Produces(200)
        .CacheOutput("Lookup60s");

        // GET /api/v1/accounting/withholding-rates — stopaj oranları
        group.MapGet("/withholding-rates", async (ISender mediator, CancellationToken ct) =>
        {
            var result = await mediator.Send(new GetWithholdingRatesQuery(), ct);
            return Results.Ok(result);
        })
        .WithName("GetWithholdingRates")
        .WithSummary("Güncel stopaj/tevkifat oranları").Produces(200)
        .CacheOutput("Lookup60s");

        // GET /api/v1/accounting/tax-withholdings — stopaj kayıtları
        group.MapGet("/tax-withholdings", async (
            Guid tenantId, DateTime? startDate, DateTime? endDate,
            ISender mediator, CancellationToken ct) =>
        {
            var result = await mediator.Send(
                new ListTaxWithholdingsQuery(tenantId, startDate, endDate), ct);
            return Results.Ok(result);
        })
        .WithName("ListTaxWithholdings")
        .WithSummary("Stopaj/tevkifat kayıtları listesi").Produces(200)
        .CacheOutput("Lookup60s");
    }
