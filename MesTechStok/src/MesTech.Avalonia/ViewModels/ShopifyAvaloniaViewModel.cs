...
public partial class ShopifyAvaloniaViewModel : ViewModelBase
{
    ...
    public ShopifyAvaloniaViewModel(IMediator mediator, ICurrentUserService currentUser)
    {
        _mediator = mediator;
    ...
    public override async Task LoadAsync()
    {
        ...
        try
        {
            var result = await _mediator.Send(new GetPlatformDashboardQuery(_currentUser.TenantId, PlatformType.Shopify)) ?? new PlatformDashboardDto();
        ...
        {
            HasError = true;
            ErrorMessage = $"Shopify verileri yuklenemedi: {ex.Message}";
        }
        finally
        {
            IsLoading = false;
    ...
    private async Task Sync()
    {
        ...
        try
        {
            var result = await _mediator.Send(new SyncPlatformCommand("Shopify", SyncDirection.Bidirectional));
            if (result.IsSuccess)
            {
