...
public partial class PttAvmAvaloniaViewModel : ViewModelBase
{
    ...
    public PttAvmAvaloniaViewModel(IMediator mediator, ICurrentUserService currentUser)
    {
        _mediator = mediator;
    ...
    public override async Task LoadAsync()
    {
        ...
        try
        {
            var result = await _mediator.Send(new GetPlatformDashboardQuery(_currentUser.TenantId, PlatformType.PttAVM)) ?? new PlatformDashboardDto();
        ...
        {
            HasError = true;
            ErrorMessage = $"PTT AVM verileri yuklenemedi: {ex.Message}";
        }
        finally
        {
            IsLoading = false;
    ...
    private async Task Sync()
    {
        ...
        try
        {
            var result = await _mediator.Send(new SyncPlatformCommand("PttAvm", SyncDirection.Bidirectional));
            if (result.IsSuccess)
            {
