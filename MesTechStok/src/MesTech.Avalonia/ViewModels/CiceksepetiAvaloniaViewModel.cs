...
public partial class CiceksepetiAvaloniaViewModel : ViewModelBase
{
    ...
    public CiceksepetiAvaloniaViewModel(IMediator mediator, ICurrentUserService currentUser)
    {
        _mediator = mediator;
    ...
    public override async Task LoadAsync()
    {
        ...
        try
        {
            var result = await _mediator.Send(new GetPlatformDashboardQuery(_currentUser.TenantId, PlatformType.Ciceksepeti)) ?? new PlatformDashboardDto();
        ...
        {
            HasError = true;
            ErrorMessage = $"Ciceksepeti verileri yuklenemedi: {ex.Message}";
        }
        finally
        {
            IsLoading = false;
    ...
    private async Task Sync()
    {
        ...
        try
        {
            var result = await _mediator.Send(new SyncPlatformCommand("Ciceksepeti", SyncDirection.Bidirectional));
            if (result.IsSuccess)
            {
