...
public partial class N11AvaloniaViewModel : ViewModelBase
{
    ...
    public override async Task LoadAsync()
    {
        ...
        {
            var result = await _mediator.Send(new GetPlatformDashboardQuery(_currentUser.TenantId, PlatformType.N11), ct) ?? new PlatformDashboardDto();
    ...
    private async Task Sync()
    {
        ...
        try
        {
            var result = await _mediator.Send(new SyncPlatformCommand("N11", SyncDirection.Bidirectional));
            if (result.IsSuccess)
            {
