...
public partial class WooCommerceAvaloniaViewModel : ViewModelBase
{
    ...
    public WooCommerceAvaloniaViewModel(IMediator mediator, ICurrentUserService currentUser)
    {
        _mediator = mediator;
    ...
    public override async Task LoadAsync()
    {
        ...
        try
        {
            var result = await _mediator.Send(new GetPlatformDashboardQuery(_currentUser.TenantId, PlatformType.WooCommerce)) ?? new PlatformDashboardDto();
        ...
        {
            HasError = true;
            ErrorMessage = $"WooCommerce verileri yuklenemedi: {ex.Message}";
        }
        finally
        {
            IsLoading = false;
