...
public partial class EtsyAvaloniaViewModel : ViewModelBase
{
    ...
    public EtsyAvaloniaViewModel(IMediator mediator, ICurrentUserService currentUser)
    {
        _mediator = mediator;
    ...
    public override async Task LoadAsync()
    {
        ...
        try
        {
            var result = await _mediator.Send(new GetPlatformDashboardQuery(_currentUser.TenantId, PlatformType.Etsy)) ?? new PlatformDashboardDto();
        ...
        {
            HasError = true;
            ErrorMessage = $"Etsy verileri yuklenemedi: {ex.Message}";
        }
        finally
        {
            IsLoading = false;
    ...
    private async Task Sync()
    {
        ...
        try
        {
            var result = await _mediator.Send(new SyncPlatformCommand("Etsy", SyncDirection.Bidirectional));
            if (result.IsSuccess)
            {
