...
public partial class OzonAvaloniaViewModel : ViewModelBase
{
    ...
    public OzonAvaloniaViewModel(IMediator mediator, ICurrentUserService currentUser)
    {
        _mediator = mediator;
    ...
    public override async Task LoadAsync()
    {
        ...
        try
        {
            var result = await _mediator.Send(new GetPlatformDashboardQuery(_currentUser.TenantId, PlatformType.Ozon)) ?? new PlatformDashboardDto();
            ...
            foreach (var o in result.RecentOrders)
                _allOrders.Add(new PlatformOrderItem(o.OrderNumber, o.OrderDate.ToString("dd.MM.yyyy"), o.CustomerName, o.Total.ToString("N2"), o.Status, PlatformType.Ozon));
            ApplyFilter();
        }
        catch (Exception ex)
        {
            HasError = true;
            ErrorMessage = $"Ozon verileri yuklenemedi: {ex.Message}";
        }
        finally
        {
            IsLoading = false;
    ...
    private async Task Sync()
    {
        ...
        try
        {
            var result = await _mediator.Send(new SyncPlatformCommand("Ozon", SyncDirection.Bidirectional));
            if (result.IsSuccess)
            {
