...
public interface ITenantSubscriptionRepository
{
    ...
    Task<IReadOnlyList<TenantSubscription>> GetDueForRenewalAsync(DateTime asOfDate, CancellationToken ct = default);
    Task<IReadOnlyList<TenantSubscription>> GetByStatusAsync(SubscriptionStatus status, CancellationToken ct = default);
