...
public class MesaMeetingScheduledConsumer : IConsumer<MesaMeetingScheduledEvent>
{
    ...
    public async Task Consume(ConsumeContext<MesaMeetingScheduledEvent> context)
    {
        ...
        _logger.LogInformation(
            "Processing {Consumer} MessageId={MessageId} — '{Title}' {StartAt}",
        ...
        {
            _logger.LogError(ex, "{Consumer} failed for MessageId={MessageId}",
                nameof(MesaMeetingScheduledConsumer), context.MessageId);
            throw; // MassTransit retry policy handles
        }
    }
}
