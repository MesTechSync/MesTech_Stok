...
public sealed class ELogoInvoiceProvider : IInvoiceProvider, IBulkInvoiceCapable, IIncomingInvoiceCapable, IKontorCapable, IEInvoiceProvider
{
    ...
    async Task<bool> IEInvoiceProvider.CancelAsync(string providerRef, string reason, CancellationToken ct)
    {
        ...
        {
            _logger.LogError(ex, "[ELogo] CancelAsync exception ref={Ref}", providerRef);
            return false;
        }
    }
    ...
    async Task<VknMukellefResult> IEInvoiceProvider.CheckVknMukellefAsync(string vkn, CancellationToken ct)
    {
        ...
        try
        {
            using var response = await _httpClient.GetAsync($"{_baseUrl}/api/mukellef/{vkn}", ct).ConfigureAwait(false);
            if (!response.IsSuccessStatusCode)
                return new VknMukellefResult(vkn, false, false, null, DateTime.UtcNow);
            var body = await response.Content.ReadAsStringAsync(ct).ConfigureAwait(false);
            using var doc = JsonDocument.Parse(body);
            var root = doc.RootElement;
            var isEInvoice = root.TryGetProperty("efaturaKullanici", out var ef) && ef.GetBoolean();
            var isEArchive = root.TryGetProperty("earsivKullanici", out var ea) && ea.GetBoolean();
            var title = root.TryGetProperty("unvan", out var tv) ? tv.GetString()
        ...
        {
            _logger.LogError(ex, "[ELogo] CheckVknMukellefAsync exception vkn={Vkn}", vkn);
            return new VknMukellefResult(vkn, false, false, null, DateTime.UtcNow);
        }
    }
    ...
    async Task<int> IEInvoiceProvider.GetCreditBalanceAsync(CancellationToken ct)
    {
        ...
        try
        {
            using var response = await _httpClient.GetAsync($"{_baseUrl}/api/account/kontor", ct).ConfigureAwait(false);
            if (!response.IsSuccessStatusCode) return -1;
            var body = await response.Content.ReadAsStringAsync(ct).ConfigureAwait(false);
            using var doc = JsonDocument.Parse(body);
            return doc.RootElement.TryGetProperty("kontor", out var k) ? k.GetInt32()
                 : doc.RootElement.TryGetProperty("balance", out var b) ? b.GetInt32() : -1;
        }
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            _logger.LogError(ex, "[ELogo] GetCreditBalanceAsync exception");
            return -1;
        }
    }
}
