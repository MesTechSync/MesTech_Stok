...
public sealed class ArasKargoAdapter : ICargoAdapter, ICargoRateProvider
{
    ...
    public ArasKargoAdapter(HttpClient httpClient, ILogger<ArasKargoAdapter> logger)
    {
        ...
        _retryPipeline = new ResiliencePipelineBuilder<HttpResponseMessage>()
            ...
            .AddRetry(new RetryStrategyOptions<HttpResponseMessage>
            {
                ...
                OnRetry = args =>
                {
                    _logger.LogWarning("[ArasKargo] Rate limited (429). Retry {Attempt} after {Delay}ms",
            ...
            .AddRetry(new RetryStrategyOptions<HttpResponseMessage>
            {
                ...
                OnRetry = args =>
                {
                    _logger.LogWarning("Aras Kargo API retry {Attempt} after {Delay}ms",
                        args.AttemptNumber, args.RetryDelay.TotalMilliseconds);
                    return default;
                }
            })
            .AddCircuitBreaker(new CircuitBreakerStrategyOptions<HttpResponseMessage>
            {
                ...
                OnOpened = args =>
                {
                    _logger.LogWarning("ArasKargo circuit breaker OPENED for {Duration}s",
    ...
    public CargoProvider Provider => CargoProvider.ArasKargo;
    ...
    {
        if (!_isConfigured)
            throw new InvalidOperationException("ArasKargoAdapter henuz konfigure edilmedi.");
    }
    ...
    public async Task<bool> IsAvailableAsync(CancellationToken ct = default)
    {
        ...
        {
            _logger.LogWarning(ex, "[ArasKargoAdapter] IsAvailableAsync health check exception");
    ...
    public async Task<ShipmentResult> CreateShipmentAsync(ShipmentRequest request, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogWarning("Aras Kargo CreateShipment failed {Status}: {Error}",
                    response.StatusCode, error);
                return ShipmentResult.Failed($"Aras Kargo hatasi: {response.StatusCode}");
            }
            ...
            if (string.IsNullOrEmpty(trackingNo))
                return ShipmentResult.Failed("Aras Kargo tracking number alinamadi");

            _logger.LogInformation("Aras Kargo shipment created: {TrackingNo}", trackingNo);
            return ShipmentResult.Succeeded(trackingNo, shipmentId ?? trackingNo);
        }
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            _logger.LogError(ex, "Aras Kargo CreateShipment failed for order {OrderId}", request.OrderId);
            return ShipmentResult.Failed($"Aras Kargo hatasi: {ex.Message}");
        }
    }
    ...
    public async Task<TrackingResult> TrackShipmentAsync(string trackingNumber, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogWarning("Aras Kargo tracking failed {Status} {Error}", response.StatusCode, errorBody);
            ...
            if (doc.RootElement.TryGetProperty("status", out var statusProp))
                trackingResult.Status = MapArasStatus(statusProp.GetString() ?? "");
                ...
                foreach (var evt in events.EnumerateArray())
                {
                    trackingResult.Events.Add(new TrackingEvent
                    {
                        ...
                        Status = evt.TryGetProperty("status", out var st)
                            ? MapArasStatus(st.GetString() ?? "") : CargoStatus.Created
                    });
                }
            }
        }
        ...
        {
            _logger.LogError(ex, "Aras Kargo tracking failed for {TrackingNumber}", trackingNumber);
    ...
    public async Task<bool> CancelShipmentAsync(string shipmentId, CancellationToken ct = default)
    {
        ...
        if (response.IsSuccessStatusCode)
        {
            _logger.LogInformation("Aras Kargo shipment cancelled: {ShipmentId}", shipmentId);
            return true;
        }

        _logger.LogWarning("Aras Kargo cancel failed {Status}", response.StatusCode);
        return false;
    }
    ...
    public async Task<LabelResult> GetShipmentLabelAsync(string shipmentId, CancellationToken ct = default)
    {
        ...
        {
            var errorBody = await response.Content.ReadAsStringAsync(ct).ConfigureAwait(false);
            throw new HttpRequestException($"Aras Kargo label request failed: {response.StatusCode} — {errorBody}");
        }
        ...
        if (string.IsNullOrEmpty(base64))
            throw new InvalidOperationException("Aras Kargo etiket verisi bos");

        return new LabelResult
        {
            Data = Convert.FromBase64String(base64),
            Format = LabelFormat.Pdf,
            FileName = $"aras-label-{shipmentId}.pdf"
        };
    }
    ...
    private static CargoStatus MapArasStatus(string status) => status.ToLowerInvariant() switch
    {
