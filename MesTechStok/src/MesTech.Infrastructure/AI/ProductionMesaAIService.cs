...
public class ProductionMesaAIService : IMesaAIService
{
        ...
        List<string>? imageUrls, CancellationToken ct = default)
    {
        try
        {
            ...
            {
                _logger.LogWarning(
                    "[MESA Prod] GenerateDescription failed: {StatusCode} — falling back to mock (sku={SKU})",
            ...
            {
                _logger.LogWarning(
                    "[MESA Prod] GenerateDescription deserialization failed — falling back (sku={SKU})", sku);
                return await _mockFallback.GenerateProductDescriptionAsync(
                    sku, productName, category, imageUrls, ct);
            }

            _logger.LogInformation(
                "[MESA Prod] GenerateDescription basarili: sku={SKU}, len={Len}",
        ...
        {
            _logger.LogError(ex,
                "[MESA Prod] MESA OS unreachable, falling back to mock (GenerateDescription, sku={SKU})", sku);
            return await _mockFallback.GenerateProductDescriptionAsync(
                sku, productName, category, imageUrls, ct);
        }
    }
        ...
        string sku, List<string> sourceImageUrls, CancellationToken ct = default)
    {
        try
        {
            ...
            {
                _logger.LogWarning(
                    "[MESA Prod] GenerateImages failed: {StatusCode} — falling back (sku={SKU})",
                    response.StatusCode, sku);
            ...
            {
                _logger.LogWarning(
                    "[MESA Prod] GenerateImages empty response — falling back (sku={SKU})", sku);
                return await _mockFallback.GenerateProductImagesAsync(sku, sourceImageUrls, ct);
            }

            _logger.LogInformation(
                "[MESA Prod] GenerateImages basarili: sku={SKU}, count={Count}",
        ...
        {
            _logger.LogError(ex,
                "[MESA Prod] MESA OS unreachable, falling back to mock (GenerateImages, sku={SKU})", sku);
            return await _mockFallback.GenerateProductImagesAsync(sku, sourceImageUrls, ct);
        }
    }
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            {
                _logger.LogWarning(
                    "[MESA Prod] RecommendPrice failed: {StatusCode} — falling back (sku={SKU})",
                    response.StatusCode, sku);
            ...
            {
                _logger.LogWarning(
                    "[MESA Prod] RecommendPrice null response — falling back (sku={SKU})", sku);
                return await _mockFallback.RecommendPriceAsync(sku, currentPrice, competitors, ct);
            }

            _logger.LogInformation(
                "[MESA Prod] RecommendPrice basarili: sku={SKU}, oneri={Price:F2}",
        ...
        {
            _logger.LogError(ex,
                "[MESA Prod] MESA OS unreachable, falling back to mock (RecommendPrice, sku={SKU})", sku);
            return await _mockFallback.RecommendPriceAsync(sku, currentPrice, competitors, ct);
        }
    }
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            {
                _logger.LogWarning(
                    "[MESA Prod] PredictStock failed: {StatusCode} — falling back (sku={SKU})",
                    response.StatusCode, sku);
            ...
            {
                _logger.LogWarning(
                    "[MESA Prod] PredictStock null response — falling back (sku={SKU})", sku);
                return await _mockFallback.PredictStockAsync(sku, currentStock, history, ct);
            }

            _logger.LogInformation(
                "[MESA Prod] PredictStock basarili: sku={SKU}, demand={Demand}, days={Days}",
        ...
        {
            _logger.LogError(ex,
                "[MESA Prod] MESA OS unreachable, falling back to mock (PredictStock, sku={SKU})", sku);
            return await _mockFallback.PredictStockAsync(sku, currentStock, history, ct);
        }
    }
        ...
        string targetPlatform, CancellationToken ct = default)
    {
        try
        {
            ...
            {
                _logger.LogWarning(
                    "[MESA Prod] SuggestCategory failed: {StatusCode} — falling back (product={Name})",
            ...
            {
                _logger.LogWarning(
                    "[MESA Prod] SuggestCategory null response — falling back (product={Name})", productName);
                return await _mockFallback.SuggestCategoryAsync(
                    productName, description, targetPlatform, ct);
            }

            _logger.LogInformation(
                "[MESA Prod] SuggestCategory basarili: {Name} -> {CategoryId} ({Confidence:P0})",
        ...
        {
            _logger.LogError(ex,
                "[MESA Prod] MESA OS unreachable, falling back to mock (SuggestCategory, product={Name})",
                productName);
            return await _mockFallback.SuggestCategoryAsync(
                productName, description, targetPlatform, ct);
        }
