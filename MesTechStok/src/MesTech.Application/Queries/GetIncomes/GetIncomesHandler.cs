...
public sealed class GetIncomesHandler : IRequestHandler<GetIncomesQuery, IReadOnlyList<IncomeDto>>
{
    ...
    public async Task<IReadOnlyList<IncomeDto>> Handle(GetIncomesQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            var byType = await _incomeRepository.GetByTypeAsync(request.Type.Value, request.TenantId, cancellationToken).ConfigureAwait(false);
            return byType.Adapt<List<IncomeDto>>().AsReadOnly();
        }

        if (request.From.HasValue && request.To.HasValue)
        {
            var byRange = await _incomeRepository.GetByDateRangeAsync(request.From.Value, request.To.Value, request.TenantId, cancellationToken).ConfigureAwait(false);
            return byRange.Adapt<List<IncomeDto>>().AsReadOnly();
        }

        var all = await _incomeRepository.GetAllAsync(request.TenantId, cancellationToken).ConfigureAwait(false);
