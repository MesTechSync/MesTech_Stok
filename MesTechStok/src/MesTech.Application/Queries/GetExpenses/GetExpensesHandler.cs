...
public sealed class GetExpensesHandler : IRequestHandler<GetExpensesQuery, IReadOnlyList<ExpenseDto>>
{
    ...
    public async Task<IReadOnlyList<ExpenseDto>> Handle(GetExpensesQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            var byType = await _expenseRepository.GetByTypeAsync(request.Type.Value, request.TenantId).ConfigureAwait(false);
            return byType.Adapt<List<ExpenseDto>>().AsReadOnly();
        }

        if (request.From.HasValue && request.To.HasValue)
        {
            var byRange = await _expenseRepository.GetByDateRangeAsync(request.From.Value, request.To.Value, request.TenantId).ConfigureAwait(false);
            return byRange.Adapt<List<ExpenseDto>>().AsReadOnly();
        }

        var all = await _expenseRepository.GetAllAsync(request.TenantId).ConfigureAwait(false);
